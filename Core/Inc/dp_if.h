...
/*---------------------------------------------------------------------------*/
   ...
   /* ISR Logging functions (non-ISR safe) */
   #if VPC3_SERIAL_MODE
      extern uint8_t dp_isr_read_log     ( char* buffer, uint8_t max_length );
      extern void    dp_isr_clear_log    ( void );
      extern uint8_t dp_isr_log_overflow ( void );
   #else
      /* No-op macros cuando el logger no está activo */
      #define dp_isr_read_log(buffer, max_len)     (0)
      #define dp_isr_clear_log()                   ((void)0)
      #define dp_isr_log_overflow()                (0)
   #endif

   /* DpCfg Logging functions (ISR-safe, integradas con ring buffer del ISR) */
   #if VPC3_SERIAL_MODE
